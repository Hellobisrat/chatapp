       ...
       {messages?.map((message,index)=>(
        ...
        ${message.senderId !== authUser._id && 'flex-row-reverse'}`}>
          {message.image ? (
            ...
            <p className={` p-2 max-w-[200px] md:text-sm font-light mb-8 break-all bg-violet-500/30  text-black text-xs ${message.senderId=== authUser._id ? 'rounded-br-none':
              'rounded-bl-none'}`}>{message.text}</p>
          )}
          <div className='text-center text-xs'>
            <img src={message.senderId=== authUser._id ? authUser?.profilePic || assets.avatar_icon: selectedUser?.profilePic || assets.arrow_icon} alt=''
